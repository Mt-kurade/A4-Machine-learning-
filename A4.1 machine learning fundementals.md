...
Regression-based and classification-based learning models vary based on the desired output. Regression algorithms provide a continous output from a range which is numerical based. While, a classifier sorts a input into a label it was trained on. 
...
This wou be an example for supervised learning. 
#### b Describe your reasoning for this choice.
For emails to be categorized into labels, the inputs provided to the model need to be labels under the specific folder for the algorithm to accuretly sort future incoming emails.
...
Transfer learning is a machine learning technique in which a model trained on one task is reused and adapted to perform a different but related task. It involves transferring the knowledged (such as learned feature or patterns) from the original model to improve the leraning efficiency and performance on the new task, often reducing the amount of data and training time required. 


#### b Outline how this is an example of transfer learning.
A model trained to navigate through one city used to design a new model to navigate another city would be an example of transfer learning. Since the model frm
...
computer dundemtnatl 
db 
machine learning (2)
opp 
B 1, 2, 3 (no abtract data types
